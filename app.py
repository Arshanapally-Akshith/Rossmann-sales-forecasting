...
if "files_downloaded" not in st.session_state:
    download_file("13Wec_HB9AZ6RyOIT9LLGaUAJs997CUaZ", "reconciled_predictions_app.csv")
    download_file("1nZRYgPIoxFrkDb4K-hCTeGGn1HU0eBMg", "cleaned_rossmann_app.csv")
    download_file("17nEPSrwnBXiyXyFKy3RNpk40h1yw09o2", "sql_features_app.csv")
    st.session_state["files_downloaded"] = True


@st.cache_data
def load_reconciled():
    df = pd.read_csv("reconciled_predictions_app.csv", parse_dates=["Date"])
...
@st.cache_data
def load_cleaned():
    return pd.read_csv("cleaned_rossmann_app.csv", parse_dates=["Date"])

@st.cache_data
def load_sql():
    return pd.read_csv("sql_features_app.csv", parse_dates=["Date"])

@st.cache_data
def load_weekly():
    # Pre-compute weekly RMSE from reconciled predictions
    df = pd.read_csv("reconciled_predictions_app.csv", parse_dates=["Date"])
    df["Week_dt"] = df["Date"] - pd.to_timedelta(df["Date"].dt.dayofweek, unit="d")
    weekly = (
        df.groupby("Week_dt")
    ...
    weekly["Week"] = weekly["Week_dt"].dt.strftime("%Y-W%V").astype(str)
    return weekly[["Week", "RMSE"]]

@st.cache_data
def load_risk():
    # Compute top 5 stockout risk stores from reconciled predictions
    df = pd.read_csv("reconciled_predictions_app.csv", parse_dates=["Date"])
    df["pct_error"] = (df["Sales"] - df["reconciled_pred"]).abs() / df["Sales"].clip(lower=1)
    df["is_anomaly"] = df["pct_error"] > df["pct_error"].quantile(0.995)
    df["Week_dt"] = df["Date"] - pd.to_timedelta(df["Date"].dt.dayofweek, unit="d")
    last_4_weeks = df["Week_dt"].max() - pd.Timedelta(weeks=4)
    recent_anom  = df[(df["is_anomaly"]) & (df["Week_dt"] > last_4_weeks)]
    top5 = recent_anom["Store"].value_counts().head(5).reset_index()
    top5.columns = ["Store", "Anomaly_Count_Last_4_Weeks"]
    top5["Risk"] = top5["Anomaly_Count_Last_4_Weeks"].apply(
...
# ─────────────────────────────────────────────────────────────
with st.sidebar:
    st.image("https://img.shields.io/badge/Rossmann-Supply%20Chain-blue", width=200)
...
# ════════════════════════════════════════════════════════════
    ...
    with col_l:
        ...
        if not s2_df.empty and s2_df["Date"].max() >= test_start:
            rect_x0 = max(test_start, s2_df["Date"].min())
            rect_x1 = s2_df["Date"].max()
            fig9.add_vrect(
...
# ════════════════════════════════════════════════════════════
    ...
    adf = df_recon[df_recon["Year"].isin(year_filter)].copy()
