...
# Load all data — weekly and risk computed from df_recon to avoid re-reading the file
with st.spinner("Loading data..."):
    ...
    df_weekly = (
        df_recon.groupby("Week_dt")
        .apply(lambda x: np.sqrt(np.mean((x["Sales"] - x["reconciled_pred"])**2)))
        .reset_index(name="RMSE")
    )
    df_weekly["Week"] = df_weekly["Week_dt"].dt.strftime("%Y-W%V").astype(str)
    df_weekly = df_weekly[["Week", "RMSE"]]

    last_4_weeks = df_recon["Week_dt"].max() - pd.Timedelta(weeks=4)
    recent_anom  = df_recon[(df_recon["is_anomaly"]) & (df_recon["Week_dt"] > last_4_weeks)]
